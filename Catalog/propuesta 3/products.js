...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("close-lost-ticket").addEventListener("click", () => {
    ...
    document.getElementById("recover-ticket-form").reset()
  })

  // Agregar evento para el botón de recuperar ticket
  document.getElementById("recover-button").addEventListener("click", async (e) => {
    e.preventDefault()
  ...
  // Agregar eventos para los botones de cancelar y aplazar pedido
  document.getElementById("cancel-order-button").addEventListener("click", async () => {
    await handleOrderAction("cancelar")
  })

  document.getElementById("extend-order-button").addEventListener("click", async () => {
    await handleOrderAction("aplazar")
  })

  // Función para manejar las acciones de cancelar o aplazar pedido
  async function handleOrderAction(action) {
    // Obtener los datos del formulario
    const orderId = document.getElementById("recover-id").value.trim()
    const name = document.getElementById("recover-name").value.trim().toUpperCase()
    const email = document.getElementById("recover-email").value.trim()
    const phone = document.getElementById("recover-phone").value.trim()
    ...
    try {
      ...
      if (querySnapshot.empty) {
        showOrderMessage("error", "No se encontró ningún pedido con los datos proporcionados.")
        return
      }
      ...
      if (orderData.estado === "cancelado" && action === "cancelar") {
        showOrderMessage("error", "Este pedido ya ha sido cancelado.")
        return
      }

      // Verificar si el pedido ya ha sido aplazado
      if (orderData.aplazado && action === "aplazar") {
        showOrderMessage(
          "error",
          "Este pedido ya ha sido aplazado anteriormente. Solo se permite un aplazamiento por pedido.",
        )
        return
      }
      ...
      const now = new Date()
      const expiracion = orderData.EXPIRACION.toDate()

      if (now > expiracion) {
        showOrderMessage("error", "Este pedido ya ha expirado y no puede ser modificado.")
        return
      }
      ...
      if (action === "cancelar") {
        ...
        // Eliminar el documento del boleto completamente
        await deleteDoc(doc(db, "Boleto", orderDoc.id))

        showOrderMessage("success", "El pedido ha sido cancelado y eliminado exitosamente.")
      } else if (action === "aplazar") {
        // Calcular nueva fecha de expiración (24 horas adicionales)
        const nuevaExpiracion = new Date(expiracion)
        nuevaExpiracion.setHours(nuevaExpiracion.getHours() + 24)

        // Aplazar el pedido
        await updateDoc(doc(db, "Boleto", orderDoc.id), {
          EXPIRACION: Timestamp.fromDate(nuevaExpiracion),
          aplazado: true,
        })

        showOrderMessage("success", "El pedido ha sido aplazado por 24 horas adicionales.")
      }

      // Limpiar el formulario después de una acción exitosa
      setTimeout(() => {
        document.getElementById("recover-ticket-form").reset()
      }, 3000)
    } catch (error) {
      console.error("Error al procesar la acción:", error)
      showOrderMessage("error", "Ocurrió un error al procesar tu solicitud. Por favor, intenta de nuevo.")
    }
  }
